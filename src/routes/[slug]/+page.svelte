<script>
    ...
    onMount(async () => {
        if (project) {
            const mainImage = { src: project.imageURL, title: project.title.value, alt: project.description.value };
            const detailImages = project.detailImagesStore.map(file => {
                const blob = new Blob([new Uint8Array(file.data)], { type: file.contentType });
                return { src: URL.createObjectURL(blob), title: file.name, alt: file.name };
            });
...
{#if project}
    <div class="mt-8">
        {#if imagesLoaded}
            ...
            <DetailPageHeading title={project.title.value} githubLink={project.github.value} />
